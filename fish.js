...
let fish, headBone, vertebraeBones = [], tailBone;
let mixer;

loader.load(
  "assets/fishBones-computer.glb",
  (gltf) => {
    fish = gltf.scene;
    ...
    if (gltf.animations && gltf.animations.length) {
      ...
      console.log(`Animation loaded: ${gltf.animations[0].name}, duration: ${gltf.animations[0].duration}s`);
    }
...
function animate() {
  ...
  if (headBone) {
    ...
    const clampedPitch = THREE.MathUtils.clamp(pitch * maxPitch, -maxPitch, maxPitch);
